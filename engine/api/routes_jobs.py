...
def progresso(
    ...
    with conexao.cursor() as cursor:
        ...
        if tarefa["state"] in PENDENTES and db.start_job(cursor, str(job_id)):
            try:
                _processa(
                    cursor, tarefa, livro, str(job_id), leitor.id, settings, embedder, authorization
                )
            except Exception as erro:
                codigo = getattr(erro, "code", "internal_error")
                db.fail_job(cursor, job_id=str(job_id), error_code=codigo)
                db.fail_book(cursor, book_id=str(tarefa["book_id"]))
                raise
        depois = db.get_job(cursor, str(job_id)) or tarefa
        return _progresso(depois)


def _processa(cursor, tarefa, livro, job_id, owner_id, settings, embedder, authorization) -> None:
    ...
    """
    dados = storage.download(
        settings.supabase_url, livro["storage_path"], token=authorization or ""
    )
